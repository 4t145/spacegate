...
impl Layer<SgBoxService> for HeaderModifierLayer {
    ...
    fn layer(&self, service: SgBoxService) -> Self::Service {
        ...
        let req_map_layer = MapRequestLayer::new(move |req: SgRequest| {
            let mut req = req;
            for (k, v) in &req_filter.sets {
                req.request.headers_mut().append(k, v.clone());
            }
            for k in &req_filter.remove {
                req.request.headers_mut().remove(k);
            }
            req
        });
        let resp_map_layer = MapResponseLayer::new(move |resp: SgResponse| {
            let mut resp = resp;
            for (k, v) in resp_filter.sets.iter() {
                resp.response.headers_mut().append(k, v.clone());
            }
            for k in &resp_filter.remove {
                resp.response.headers_mut().remove(k);
            }
            resp
        });
